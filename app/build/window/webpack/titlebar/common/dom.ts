...
const _manualClassList = new class implements IDomClassList {
	...
	private _lastStart: number | undefined;
	private _lastEnd: number | undefined;
	...
	removeClass(node: HTMLElement, className: string): void {
		...
		if (this._lastStart === -1) {
			...
			node.className = node.className.substring(0, this._lastStart) + node.className.substring(this._lastEnd ?? -1);
		}
	}
